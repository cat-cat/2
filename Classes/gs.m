...
+ (NSString*)db_GetLastUpdate
{
    ...
    sqlStatement = sqlite3_mprintf("SELECT id"
                                   " FROM [updates]"
                                   " ORDER BY id DESC"
                                   " LIMIT 0,1");
    ...
    returnCode = sqlite3_step(statement);
    while(returnCode == SQLITE_ROW){
        // MUST always be something in the result or error
        fieldID = [NSString stringWithCString:(char*)sqlite3_column_text(statement, 0) encoding:NSUTF8StringEncoding];
        returnCode = sqlite3_step(statement);                
    }
    returnCode = sqlite3_finalize(statement);
    [gs assertNoError:returnCode==SQLITE_OK withMsg:[NSString stringWithFormat:@"Cannot finalize %s", sqlite3_errmsg(db) ]];
...
//        
//        
//        DDXMLDocument *doc = [self docForPage:@"hasUpdate2.php"];
//        if (!doc) {
...
//        {
//            doc = [self docForPage:@"getAbookCatalogUpdate.php"];
//            if (!doc) {
