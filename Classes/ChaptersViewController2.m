...
#import "ChaptersViewController2.h"
#import "gs.h"
...
@interface ChaptersViewController2 ()

@end

static NSString* BTN_READY2 = @"Готово";
static NSString* BTN_DOWNLOAD2 = @"Скачать";
static NSString* BTN_CANCEL2 = @"отменить";
...
@implementation ChaptersViewController2
//@synthesize bookId;
- (void)requestBookMeta:(NSString*)bid
{
    NSString *devid = [OpenUDID value];
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://%@/v2/bookmeta.php?bid=%@&dev=%@", BookHost, bid, devid]];
...
- (void) updateMeta:(NSString*) fileContent
{
    ...
    for (DDXMLElement *item in items) {
        ...
        Chapter2* c = [[Chapter2 alloc] init];
        c.name = cName;
        c.cId = cId;
...
//    if ([isbought isEqualToString:@"yes"]) {
    ...
    NSString *fileName = [gss() pathForBuy:[StaticPlayer2 sharedInstance].bookID];
...
{    
    ...
    NSString *pathToBookDirectory =[ [[gs sharedInstance] dirsForBook:[StaticPlayer2 sharedInstance].bookID] stringByAppendingPathComponent:@"bookMeta.xml"];
...
//    bookId = [PlayerViewController myGetBookId];
    ...
    NSString* fMetaPath = [NSString stringWithFormat:@"%@/%@",[[gs sharedInstance] dirsForBook:[StaticPlayer2 sharedInstance].bookID ],  @"bookMeta.xml"];
    // start request to get chapters, handle answer in handlers
    if(![[NSFileManager defaultManager]  fileExistsAtPath:fMetaPath ])
    {
        [self requestBookMeta:[StaticPlayer2 sharedInstance].bookID];
    }
    else
    {
        NSError *error;
...
{    
    ...
    Chapter2* c = (Chapter2*) [chapters objectAtIndex:[[sender titleForState:UIControlStateApplication]intValue]];
    NSString* chapterIdentity = [NSString stringWithFormat:@"%@:%@", [StaticPlayer2 sharedInstance].bookID, c.cId ];
    NSString* btnState = [sender titleForState:UIControlStateNormal];
    if ([btnState isEqualToString:BTN_CANCEL2]) {
        [[StaticPlayer2 sharedInstance] removeDownqObject:chapterIdentity];
        [sender setTitle:BTN_DOWNLOAD2 forState:UIControlStateNormal];
        [sender setImage:[UIImage imageNamed:@"download.png"]  forState:UIControlStateNormal];
    }
    else if([btnState isEqualToString:BTN_DOWNLOAD2])
    {
        ...
        [PlayerViewController2 appendChapterIdentityForDownloading:chapterIdentity];
        [sender setTitle:BTN_CANCEL2 forState:UIControlStateNormal];
...
-(void)setBtnTitleForChapter:(NSString*)chid title:(NSString*)title
{
    ...
    // manage button
    if([title isEqualToString:BTN_READY2])
        btn.hidden = YES;
    else
...
-(void)chapterFinishDownload:(NSString*)chapterIdentity
{
    ...
    if (![bid isEqualToString: [StaticPlayer2 sharedInstance].bookID]) {
    ...
    float progress = [PlayerViewController2 calcDownProgressForBook:[StaticPlayer2 sharedInstance].bookID chapter:chid];
...
//    float progress = progressView.progress;
    ...
    if (progress < 1.0) {
        [self setBtnTitleForChapter:chid title:BTN_DOWNLOAD2];
        
    }
    else {
        [self setBtnTitleForChapter:chid title:BTN_READY2];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // Configure the cell...
    Chapter2 *lc = [chapters objectAtIndex:indexPath.row];
    UILabel* lblSmall = (UILabel*)[cell viewWithTag:2];
    lblSmall.text = lc.name;
    UIProgressView* progress = (UIProgressView*) [cell viewWithTag:3];
    progress.progress = [PlayerViewController2 calcDownProgressForBook:[StaticPlayer2 sharedInstance].bookID chapter:lc.cId];
    UIButton* btn = (UIButton*)[cell viewWithTag:4];
    
    if (progress.progress < 1.0) {
        if ([[StaticPlayer2 sharedInstance] downqContainsObject:[NSString stringWithFormat:@"%@:%@", [StaticPlayer2 sharedInstance].bookID, lc.cId]]) {
            [btn setTitle:BTN_CANCEL2 forState:UIControlStateNormal];
            [btn setImage:[UIImage imageNamed:@"stop.png"]  forState:UIControlStateNormal];
  
        }
        else{
            [btn setTitle:BTN_DOWNLOAD2 forState:UIControlStateNormal];
        ...
        [btn setTitle:BTN_READY2 forState:UIControlStateNormal];
        btn.hidden = YES;
        progress.hidden = YES;
    }
    DDXMLDocument *xmldoc = [gss() docForFile:[gss() pathForBookMeta:[StaticPlayer2 sharedInstance].bookID]];
    NSArray* arr = [gss() arrayForDoc:xmldoc xpath:[NSString stringWithFormat:@"//abook[@id='%@']/content/track[@number='%@']/file/length", [StaticPlayer2 sharedInstance].bookID, lc.cId]];
    
    if ([arr count] != 1) {
        NSLog(@"**err: invalid length for book: %@, chpater: %@", [StaticPlayer2 sharedInstance].bookID, lc.cId);
    }
    else
    {
...
//    if ([arr count] != 1) {
    ...
    BOOL chapterStarted = [PlayerViewController2 startChapter:chid];
    if (!chapterStarted) {
        return;
    }
    ...
    if (tableView) {
        ...
        Chapter2 *lc = [chapters objectAtIndex:indexPath.row];
        float progress = [PlayerViewController2 calcDownProgressForBook:[StaticPlayer2 sharedInstance].bookID chapter:lc.cId];
        UIButton* btn = (UIButton*)[cell viewWithTag:4];
