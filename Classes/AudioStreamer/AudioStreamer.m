...
NSString * const AS_FILE_NOT_FOUND_STRING = @"Local audio file not found.";
...
+ (NSString *)stringForErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	switch (anErrorCode)
	{
		...
		case AS_FILE_NOT_FOUND:
			return AS_FILE_NOT_FOUND_STRING;
		default:
			return AS_AUDIO_STREAMER_FAILED_STRING;
	}
...
- (BOOL)openReadStream
{
	@synchronized(self)
	{
		...
		if (!CFReadStreamOpen(stream))
		{
			...
			//[self presentAlertWithTitle:NSLocalizedStringFromTable(@"File Error", @"Errors", nil)
			//					message:NSLocalizedStringFromTable(@"Unable to configure network read stream.", @"Errors", nil)];
            NSLog(@"____denz CFReadStreamOpen error");
			return NO;
		}
		...
		if ([url.scheme isEqualToString:@"file"])
		{
			...
			{			
				...
				CFIndex length;
				NSInteger notReadedBytes = seekByteOffset;
				
				readedBytes = seekByteOffset;
				...
				@synchronized(self)
				{
					...
					while (notReadedBytes > 0)
					{
						NSInteger bufferSize = (notReadedBytes < kAQDefaultBufSize) ? notReadedBytes : kAQDefaultBufSize;
						
						length = CFReadStreamRead(stream, bytes, bufferSize);
						
						if (length == -1)
						{
							[self failWithErrorCode:AS_AUDIO_DATA_NOT_FOUND];
                            NSLog(@"____denz CFReadStreamRead error");
							return NO;
						}
						
						if (length == 0)
						{
                            NSLog(@"____denz CFReadStreamRead error zero");
							return NO;
						}
						
						notReadedBytes -= length;
					}
				}
			}
		}
...
- (void)startInternal
{
	...
	@synchronized(self)
	{
		...
		UInt32 sessionCategory = kAudioSessionCategory_LiveAudio;//kAudioSessionCategory_MediaPlayback;
...
cleanup:
        ...
        NSLog(@"_________audio streamer cleanup begin");
	@synchronized(self)
	{
		//
	...
	[pool release];
    NSLog(@"_________audio streamer cleanup ended");
}
...
- (void)internalSeekToTime:(double)newSeekTime
{
	...
	NSLog(@"Streamer filelength: %i", fileLength);
	//
...
{  
    ...
    OSStatus errorMsg = AudioQueueGetParameter(audioQueue, kAudioQueueParam_Volume, &Level);
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		if ([url.scheme isEqualToString:@"file"])
		{
			//if (seekByteOffset == 0)
			{
				...
				if (nil != error)
				{
					[self failWithErrorCode:AS_FILE_NOT_FOUND];
					return;					
				}
